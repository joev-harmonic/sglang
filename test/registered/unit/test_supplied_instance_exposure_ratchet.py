        ...
        for node in ast.walk(sa_class):
            ...
            for tgt in tgts:
                if (
                    ...
                    targets.add(tgt.attr)
        # The deprecated-alias normalization declares through a dict keyed by a
        # *name tuple* (`for attr in (...)`, expanded as `**renamed`), so the
        # keyword scan above sees no names; its field set is pinned here with a
        # drift guard on the tuple itself.
        alias_fields = {
            "attention_backend",
