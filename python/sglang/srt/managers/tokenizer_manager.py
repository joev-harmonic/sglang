...
class TokenizerManager(TokenizerControlMixin, TokenizerManagerScoreMixin):
    ...
    async def _ensure_paused_or_model_locked(self):
        ...
        ``_pause_notify`` is only a wakeup signal. After it fires, re-check
        ``is_pause`` while holding ``is_pause_cond`` and keep that condition
        through the update so ``continue_generation`` cannot race the update.
        """
        ...
        try:
            while True:
                ...
                async with self.is_pause_cond:
                    ...
                    # A continue raced the notification. Keep waiting for the
                    # writer or the next pause transition.
    ...
    def _create_tokenized_object(
        ...
        if isinstance(obj, GenerateReqInput):
            ...
            tokenized_obj = TokenizedGenerateReqInput(
                ...
                cache_session_id=getattr(obj, "cache_session_id", None),
